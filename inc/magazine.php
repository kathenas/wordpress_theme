...
*/
function kathenas_magazine_widgets() {
	...
	elseif ( is_page_template( 'template-magazine.php' ) && current_user_can( 'edit_theme_options' ) ) :
		...
		esc_html_e( 'Please go to Customize &#8594; Widgets and add at least one widget to the Magazine Homepage widget area.', 'kathenas' );
		echo '</p>';
...
if ( ! function_exists( 'kathenas_magazine_widget_title' ) ) :
	/**
	 ...
	 */
	function kathenas_magazine_widget_title( $widget_title, $category_id ) {

		// Check if widget shows a specific category.
		if ( $category_id > 0 ) {

			// Set URL and Title for Category.
			$category_title = sprintf( esc_html__( 'View all posts from category %s', 'kathenas' ), get_cat_name( $category_id ) );
...
if ( ! function_exists( 'kathenas_magazine_entry_meta' ) ) :
	/**
	 * Displays the date and author of magazine posts
	 */
	function kathenas_magazine_entry_meta() {
...
if ( ! function_exists( 'kathenas_magazine_entry_date' ) ) :
	/**
	 * Displays the date of magazine posts
	 */
	function kathenas_magazine_entry_date() {
		echo '<div class="entry-meta">' . kathenas_meta_date() . '</div>';
	}
endif;
...
* @return int
*/
function kathenas_magazine_posts_excerpt_length( $length ) {
	return 12;
}
...
*/
function kathenas_get_magazine_post_ids( $cache_id, $category, $number_of_posts ) {

	$cache_id = sanitize_key( $cache_id );
	$post_ids = get_transient( 'kathenas_magazine_post_ids' );
	...
	if ( ! isset( $post_ids[ $cache_id ] ) || is_customize_preview() ) {
		...
		// Set Transient.
		set_transient( 'kathenas_magazine_post_ids', $post_ids );
	}

	return apply_filters( 'kathenas_magazine_post_ids', $post_ids[ $cache_id ], $cache_id );
}
...
* @return void
*/
function kathenas_flush_magazine_post_ids() {
	delete_transient( 'kathenas_magazine_post_ids' );
}
add_action( 'save_post', 'kathenas_flush_magazine_post_ids' );
add_action( 'deleted_post', 'kathenas_flush_magazine_post_ids' );
add_action( 'customize_save_after', 'kathenas_flush_magazine_post_ids' );
add_action( 'switch_theme', 'kathenas_flush_magazine_post_ids' );
